...
ggplot(data = vgm0, aes(x = vt, y = vv)) +
  ...
  stat_summary(fun = mean, geom = 'line', color = 'purple3', size = 1, na.rm = TRUE) + 
  geom_hline(yintercept = vgm0$sigma2) +
  theme_light() +
...
Poblacional: $\beta_{0i}=B_1 + B_2*D_{1i} + B_3*D_{2i} + b_{0i} \\ \beta_{1i}=B_4 + B_5*D_{1i} + B_6*D_{2i} \\ \beta_{2i}=B_7 + B_8*D_{1i} + B_9*D_{2i}$

Reestimamos el modelo usando ML

```{r}
m1.exp2 = update(m1.exp, 
                 method = 'ML')
summary(m1.exp2)
```
...
- Con estos resultados ¿qué recomendaciones darías para el tratamiento de pacientes con micosis avanzada en la uña? 
...
```
Rechazo $H_0$ la long de la uña antes de comenzar el estudio depende del sexo y del club.
...
No rechazo para el club, lo que significa La longitud media de la parte no afectada por el hongo antes de comenzar el tratamiento NO está asociado con concurrir a clubes o piletas. 
...
l3.2[3,11] <- 1
l3.2[4,12] <- 1
m1.exp2.3.1 <- update(m1.exp2, fixed. = long ~ 1 + sexo + club + mes + tto:mes + club:mes + tto:club:mes)
m1.exp2.3.2 <- update(m1.exp2, fixed. = long ~ 1 + sexo + club + mes + tto:mes + sexo:mes + sexo:tto:mes)


anova(m1.exp2,m1.exp2.3.1)
anova(m1.exp2,m1.exp2.3.2)
anova(m1.exp2,L=l3.1)
anova(m1.exp2,L=l3.2)

```
No rechazo para sexo => La tasa de cambio de la longitud media de uña no afectada NO depende del sexo del paciente

Rechazo para club => La tasa de cambio de la longitud media de uña no afectada depende del hecho que concurra al gimnasio, club o pileta
...
- Con estos resultados ¿qué recomendaciones darías para el tratamiento de pacientes con micosis avanzada en la uña? 
Recomendamos utilizar Itraconazol y que en lo posible no concurra a clubes y/o piletas.